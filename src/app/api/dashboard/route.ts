...
import { assertTurmaPermission, turmaWhereByPermission } from "@/lib/rbac";
  ...
  try {
    ...
    const alunos = await prisma.aluno.findMany({
      where: {
        turma: turmaId
          ? { id: turmaId }
          : turmaWhereByPermission(actor, "VIEW_TURMA"),
      },
      select: {
        nome: true,
        entregas: {
    ...
    const resumoPorAluno: ResumoAlunoItem[] = alunos.map((aluno) => {
      const porDiaComSet = Object.fromEntries(
        days.map((day) => [day.key, { total: 0, fez: 0, falta: 0, pendentes: new Set<string>() }])
      ...
      const porDia = Object.fromEntries(
        Object.entries(porDiaComSet).map(([day, value]) => [
          day,
          {
            ...
            fez: value.fez,
            falta: value.falta,
            pendentes: Array.from(value.pendentes).sort((a, b) => a.localeCompare(b, "pt-BR")),
          },
        ])
    ...
    return errorResponse(error, "Erro ao buscar dados");
  }
}
